...
# Equalizer Class
# Apply different equalizer to audio
# API Operation                     | Description
class Equalizer:
    # Initialize audio
    # Equalizer(audio_data)
    # Input: audio_data
    # Output: None
    def __init__(self, audio_file):
        self.audio = audio_file
        with contextlib.closing(wave.open(self.audio, 'rb')) as spf:
            self.sample_Rate = spf.getframerate()
            self.amp_width = spf.getsampwidth()
            self.n_channels = spf.getnchannels()
            self.n_frames = spf.getnframes()
            # Extract Raw Audio from multi-channel Wav File
            self.signal = spf.readframes(self.n_frames * self.n_channels)
            spf.close()
            self.channels = self.interpret_wav(signal, self.n_frames, self.n_channels, self.amp_width, True)
    ...
    # Acoustic Equalizer
    def acoustic(self, order=5):
        low_cut = 85
        high_cut = 255
        low = low_cut / self.nyq
        high = high_cut / self.nyq
        b, a = signal.butter(N=order, Wn=[low, high], btype='band')
        filtered_audio = signal.lfilter(b=b, a=a, x=self.audio)
...
class Library:
    ...
    def print(self, playlist):
        if not self.linked_list:
            ...
            for item in playlist:
                ...
                print(self.library.loc[int(item)], "\n")
        else:
            playlist.print_list()
        ...
        if add_track_id in self.track_id.values:
            ...
            if self.linked_list:
                ...
                playlist.insert_after(pre_track_id=pre_track,
                                      ...
                                      file_location=str(self.file_location.loc[self.track_id.values == add_track_id].values))
                playlist.print_list()
                new_playlist = LinkedList(head=playlist.head)
                return new_playlist
            else:
                track_loc = int(self.track_id[self.track_id.values == add_track_id].index.values.astype(int)[0])
    ...
    # Input: track_id
    # Output: None
    def plot_spectrogram(self, track_id, wave_file=None, show=True, save=False, wave_plot=True):
        ...
        if wave_file is not None:
            if wave_plot:
                ...
                de_interleaved = [audio_signal[idx::wav_file.getnchannels()] for idx in range(wav_file.getnchannels())]
                ...
                if wf.getnchannels() == 1:
                    ...
                    if n % 2 > 0:  # fft odd
                        frequency_magnitude[1:len(frequency_magnitude)] = frequency_magnitude[1:len(frequency_magnitude)] * 2
                    else:  # fft even
                        frequency_magnitude[1:len(frequency_magnitude) - 1] = frequency_magnitude[1:len(frequency_magnitude) - 1] * 2
                    plt.figure()
    ...
    # Add a equalizer to the selected track
    # equalizer(audio_data, equalizer_type)
    # Input: audio_data
    # Output: modified audio data
    def equalizer(self, audio_data, equalizer_type):
...
def test_api():
    ...
    lib = Library(length=random_track_amount, file_name="raw_track_short.csv", linked_list=True)
...
#################BEGIN CORY TEST CODE
    print("*****************************************************")
    # Delete a track from the generated playlist
    delete_track_location = 55 #-1 generates error messages but then crashes ..... 55 does the same
    playlist = lib.delete_track(playlist=playlist, delete_track_location=delete_track_location)
    print("*****************************************************")
    # Add a song to the existing playlist
    add_track_id = 50     #142, -1 generates error but then crashes... 50,4 passes
    add_track_location = 4
    playlist = lib.add_track(playlist=playlist, add_track_id=add_track_id, add_track_location=add_track_location)
    print("*****************************************************")
#################END CORY TEST CODE
